function [information, solution] = GEopf_alternativeGas(mpc, options)
%
userOptions = options;

%% MATPOWER named indices
...
nGs = size(mpc.Gsou, 1);
nGasLoad = nnz(mpc.Gbus(:, 3) ~= 0);
nPTG = size(mpc.ptg, 1);
iGd = find(mpc.Gbus(:, 3) ~= 0);
gasLoadAtBuses = mpc.Gbus(iGd, 3);
...
%% User-specified gas quality limits
if isempty(userOptions)
    FSlimit = 0.2;
    WImin = 46.5;
    WImax = 52.85;
else
    FSlimit = userOptions.FSlimit;
    if userOptions.WIlimit == 1
        WImin = 47.2;
        WImax = 51.41; % old
    elseif userOptions.WIlimit == 2
        WImin = 46.5;
        WImax = 52.85;
    elseif userOptions.WIlimit == 3
        WImin = 45.8;
        WImax = 54.29;
    end
end

%% Penalty and iteration settings
alpha_PHI = 1e-1;
alpha_x = 1e-1;
alpha_Qd = 1e-1;
...
%% Warm start from the MILP relaxation
ob.Pg = [];

% step2: 初始状态认为是天然气的混合物
[warmStartInformation, warmStartSolution] = GEopf_milp(mpc);
if warmStartInformation.problem ~= 0
    error('optimization problem has issues');
end

%% Initial reference values
PGs_ref = warmStartSolution.PGs;
...
Qd_ref = repmat(gasLoadAtBuses, [1, nGasType]) .* gasComposition_ref(iGd, :);
Qgpp_ref = repmat(warmStartSolution.Qgpp, [1, nGasType]) .* gasComposition_ref(iGppGd, :);
gasFlow_sum_ref = warmStartSolution.Gf;
	
PTGbus = mpc.ptg(:, 1);
CgsPTG = sparse(PTGbus, (1:nPTG)', 1, nGb, nPTG); % connection matrix
QptgInbus_ref = CgsPTG * warmStartSolution.Qptg;
	
referenceFlowDirection = warmStartSolution.gamma;
W_ref = nodalGasInjection(PGs_ref, QptgInbus_ref, referenceFlowDirection, gasFlow_sum_ref, mpc, nGs, nGb, nGl);
gasLoad_ng = gasLoadAtBuses;

%% Sequential convex approximation loop
...
for v = 1:iterationMax
    ...
    gamma = referenceFlowDirection;

    %% Bounds and optional case-specific constraints
    Prsmin = mpc.Gbus(:, 5);
    Prsmax = mpc.Gbus(:, 6); % bar
    PGsmin = mpc.Gsou(:, 3);
    PGsmax = mpc.Gsou(:, 4); % Mm3/day
    Qptgmin = mpc.ptg(:, 5);
    Qptgmax = mpc.ptg(:, 6);
    ...
    gasCompositionEqualityCons = [];
    if isfield(mpc, 'gasCompositionEqualityBuses') && ~isempty(mpc.gasCompositionEqualityBuses)
        gasCompositionEqualityBuses = mpc.gasCompositionEqualityBuses;
        if size(gasCompositionEqualityBuses, 2) ~= 2
            error('mpc.gasCompositionEqualityBuses must be an N-by-2 matrix: [left_gas_bus_id, right_gas_bus_id].');
        end
        gasCompositionLeftRows = get_gas_bus_rows(mpc, gasCompositionEqualityBuses(:, 1), 'mpc.gasCompositionEqualityBuses');
        gasCompositionRightRows = get_gas_bus_rows(mpc, gasCompositionEqualityBuses(:, 2), 'mpc.gasCompositionEqualityBuses');
        gasCompositionEqualityCons = [gasComposition(gasCompositionLeftRows, :) == gasComposition(gasCompositionRightRows, :)];
    end
    %% Pre-calculation
    % calculate the gas flow of each pipeline
    newC = mpc.Gline(:, 3) .* sqrt(R_ng) ./ sqrt(R_pipeline_ref);
    gasDemandRefTotal = repmat(sum(Qd_ref, 2), [1, nGasType]);
    gppDemandRefTotal = repmat(sum(Qgpp_ref, 2), [1, nGasType]);
    % newC = mpc.Gline(:,3);
    ...
    % gas demand cons
    energyDemand = gasLoadAtBuses * GCV_ng; % energy need of these gas bus
    gasDemandCons = [
        -sigma_Qd <= gasComposition_ref(iGd, :) .* gasDemandRefTotal + (gasComposition(iGd, :) - gasComposition_ref(iGd, :)) .* gasDemandRefTotal ...
            + gasComposition_ref(iGd, :) .* (repmat(sum(Qd, 2), [1, nGasType]) - gasDemandRefTotal) - Qd <= sigma_Qd;
        Qd >= 0;
        sigma_Qd >= 0;
        ]:'gasDemandCons';
    ...
    GPPcons = [
        -sigma_Qgpp <= gasComposition_ref(iGppGd, :) .* gppDemandRefTotal + (gasComposition(iGppGd, :) - gasComposition_ref(iGppGd, :)) .* gppDemandRefTotal ...
            + gasComposition_ref(iGppGd, :) .* (repmat(sum(Qgpp, 2), [1, nGasType]) - gppDemandRefTotal) - Qgpp <= sigma_Qgpp;
        sigma_Qgpp >= 0;
...
%     (2./newC .* gasFlow_sum).^2 + (PHI-1).^2 <= (PHI+1).^2;
    ...
    PTGbus = mpc.ptg(:, 1);
    CgsPTG = sparse(PTGbus, (1:nPTG)', 1, nGb, nPTG); % connection matrix
    QptgInbusMethane = CgsPTG * Qptg(:, 1);
    QptgInbusHydrogen = CgsPTG * Qptg(:, 2);
    ...
    referenceInjectionByType = repmat(W_ref, [1, nGasType]);
    nodalInjectionByType = repmat(sum(nodalGasInjectionForEachComp, 2), [1, nGasType]);
    gammaPlus = repmat(1 + gamma, [1, nGasType]);
    gammaMinus = repmat(1 - gamma, [1, nGasType]);
    lineGasCompositionRef = (gammaPlus .* gasComposition_ref(mpc.Gline(:, 1), :) + gammaMinus .* gasComposition_ref(mpc.Gline(:, 2), :)) / 2;
    lineGasComposition = (gammaPlus .* gasComposition(mpc.Gline(:, 1), :) + gammaMinus .* gasComposition(mpc.Gline(:, 2), :)) / 2;
    gasFlowSumRefByType = repmat(gasFlow_sum_ref, [1, nGasType]);
    gasFlowSumByType = repmat(gasFlow_sum, [1, nGasType]);
	
    gasCompositionCons = [
        -sigma_x <= referenceInjectionByType .* gasComposition_ref + (nodalInjectionByType - referenceInjectionByType) .* gasComposition_ref ...
            + referenceInjectionByType .* (gasComposition - gasComposition_ref) - nodalGasInjectionForEachComp <= sigma_x;
        -sigma_Gf <= lineGasCompositionRef .* gasFlowSumRefByType ...
            + (lineGasComposition - lineGasCompositionRef) .* gasFlowSumRefByType ...
            + lineGasCompositionRef .* (gasFlowSumByType - gasFlowSumRefByType) ...
            - gasFlow <= sigma_Gf;
        sigma_x >= 0;
    ...
    %% Solve the convexified subproblem
    ...
    solverOptions = sdpsettings('verbose', 2, 'debug', 1, 'usex0', 0, 'gurobi.qcpdual', 1);
    if ~isempty(solverName)
        solverOptions.solver = solverName;
    end
    % solverOptions.ipopt.tol = 1e-4;
    output{v} = optimize(constraints, objfcn, solverOptions);
    if output{v}.problem ~= 0
        error('GEopf_alternativeGas failed at iteration %d with solver "%s": %s', v, solverName, output{v}.info);
    end
    ...
    if v > 1
        ...
        residualsAreSmall = (criterion.sigma_PHI(v - 1) <= 1e-3) && (criterion.sigma_x(v - 1) <= 1e-2) ...
            && (criterion.sigma_Qd(v - 1) <= 1e-2) && (criterion.sigma_Qgpp(v - 1) <= 1e-2);
        sigmaXConverged = (criterion.sigma_x(v - 1) <= gap) || (criterion.delta_sigma_x(v - 1) <= gap);
        sigmaQdConverged = (criterion.sigma_Qd(v - 1) <= gap) || (criterion.delta_sigma_Qd(v - 1) < gap);
        sigmaQgppConverged = (criterion.sigma_Qgpp(v - 1) <= gap) || (criterion.delta_sigma_Qgpp(v - 1) < gap);

        if residualsAreSmall && sigmaXConverged && sigmaQdConverged && sigmaQgppConverged
