function [information, solution] = GEopf_alternativeGas(mpc)
%
...
multiplier = 4; alpha_PHI_max = 1e4; alpha_x_max = 1e4; alpha_Qd_max = 1e4;
iterationMax = 60;
...
QptgInbus_ef = zeros(nGb,1);
signGf = GEsolution.gamma;
W_ref = nodalGasInjection(PGs_ref,QptgInbus_ef,signGf, gasFlow_sum_ref,mpc,nGs,nGb,nGl);
...
GPPcons = [
    gasComposition(iGppGd,:) .* repmat(sum(Qgpp,2),[1,nGasType]) - Qgpp == 0;
    sigma_Qgpp >= 0;
...
FScons = [
    (1-0.2) * FS_ng <= FSnodal <= (1+0.2) * FS_ng;
    ]:'FScons';
% specific gravity
SGcons = [
    S_nodal <= 0.7;
...
R_pipeline = ( (1+gamma).*R_nodal(FB) + (1-gamma).*R_nodal(TB) ) / 2;
newC_square = mpc.Gline(:,3).^2 .* R_ng ./ R_pipeline;
gasFlowSOCcons = [
...
gasCompositionCons = [
    ...
    gasFlow ./ repmat(gasFlow_sum,[1,nGasType]) == (repmat((1+gamma),[1,nGasType]).*gasComposition(mpc.Gline(:,1),:) + repmat((1-gamma),[1,nGasType]).*gasComposition(mpc.Gline(:,2),:))/2;
    sigma_x >= 0;
...
[objfcn,totalCost,genAndLCeCost,gasPurchasingCost,gasCurtailmentCost,PTGsubsidy,penalty_PHI,penalty_sinma_PHI, penalty_sigma_x] = ...
...
[sol{v}.totalCost,sol{v}.genAndLCeCost,sol{v}.gasPurchasingCost,sol{v}.gasCurtailmentCost,sol{v}.PTGsubsidy,sol{v}.penalty_PHI,sol{v}.penalty_sinma_PHI, sol{v}.penalty_sigma_x, sol{v}.objfcn, ...
    ...
    sol{v}.GCV_nodal,sol{v}.WI,sol{v}.PHI,sol{v}.sigma_PHI,sol{v}.sigma_x,sol{v}.sigma_Qd] = ...
    deal(totalCost,genAndLCeCost,gasPurchasingCost,gasCurtailmentCost,PTGsubsidy,penalty_PHI,penalty_sinma_PHI, penalty_sigma_x, objfcn, ...
    Prs_square,Prs,PGs,Qd,Qptg,Pptg,Pg,Pgpp,Qgpp,Va,LCg,gamma,gasComposition,gasFlow,gasFlow_sum,S_nodal,...
    GCV_nodal,WInodal,PHI,sigma_PHI,sigma_x,sigma_Qd);
...
if v > 1
    ...
    if (criterion.sigma_PHI(v-1)<=gap) && (criterion.sigma_x(v-1)<=gap) && (criterion.totalCost(v-1)<=gap) ...
            && (criterion.S_nodal(v-1)<=gap) && (criterion.Qd(v-1)<=gap) 
        break
    else
...
% for r = 1:nGasType
%     gasComposition_ref(:,r) = nodalGasInjectionForEachComp(PGs.*mpc.gasCompositionForGasSource(:,r),...
%         QptgInbusForAllGasComposition(:,r), gamma, gasFlow(:,r),mpc,nGs,nGb,nGl) ./ W_ref + sigma_x(:,r);
% end
...
securityIndices = [WInodal,FSnodal,GCV_nodal];
save
